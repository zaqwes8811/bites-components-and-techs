...
class MainWindow(QtGui.QMainWindow):
    ...
    def metaStateChanged(self, newState, oldState):
        ...
        # разбор файла
        pw.pw.getFileName(title)  # проходит здесь два раза
        caption = pw.pw.parseCaption()
        # заполнение структур
        artist = caption[1].__str__() 
        artistItem = QtGui.QTableWidgetItem(artist)
        artistItem.setFlags(artistItem.flags() ^ QtCore.Qt.ItemIsEditable)

        album = caption[2].__str__() 
        albumItem = QtGui.QTableWidgetItem(album)
        albumItem.setFlags(albumItem.flags() ^ QtCore.Qt.ItemIsEditable)
        
        
        year = caption[3].__str__() 
        yearItem = QtGui.QTableWidgetItem(year)  # что за входные данные
        yearItem.setFlags(yearItem.flags() ^ QtCore.Qt.ItemIsEditable)
        
        frames = caption[4].__str__() 
        framesItem = QtGui.QTableWidgetItem(frames)  # что за входные данные
        framesItem.setFlags(framesItem.flags() ^ QtCore.Qt.ItemIsEditable)
        ...
        self.musicTable.setItem(currentRow, 4, framesItem)
    ...
    def setupUi(self):
        ...
        #headers = [self.tr(u'Файл'), self.tr(u'Каналов'), self.tr("Album"), self.tr("Year")]
        headers = [u'Файл', u'Каналов',u'Разрядность, бит', u'Fs, Гц', u'Фрэймов']
        numParam = 4
        self.musicTable = QtGui.QTableWidget(0, numParam+1)
